...
J.exend(J.editor, {
	add: function(id, w, h) {
		w = w || 600;
		h = h || 200;
		var cont = this.cont = J('doc').crte('div'),
			bookmark;

		J(cont).acls('ed_c').attr('id', id + '_cont').stcs({
			width: w + 'px',
			height: h + 'px'
		}).html(
		...
		if(J.ie && !this.i8) {
			...
			if(!g) var g = new Image();
			g.onload = g.onerror = function() {
				J.editor.toolbar(id);
			};
			g.src = 'images/ed_ico.gif';
		} else this.toolbar(id);
	...
	gtrng: function(id) {
		var sel = this.ed[id].ewin.getSelection ? this.ed[id].ewin.getSelection() : this.ed[id].edoc.selection,
			rang;
	...
	_exec: function(id, c, p) {
		this.sfocu(id);
		if(J.ie) this.ed[id].rang.select();
	...
	showp: function(t, id) {
		...
		switch(t) {
			...
			case 'bcolor':
			case 'fcolor':
				var acolor = this.fcolor.toString().split(','),
					n = 0;
				...
				for(var i = 0; i < acolor.length; i++) {
					...
					if(J.ie) J(item).stcs('styleFloat', 'left');
					else J(item).stcs('cssFloat', 'left');
			...
			case 'image':
				...
				J('okbtn', this.panel._doc).click(function() {
					var url = J('url', J.editor.panel._doc).val(),
						alt = J('alt', J.editor.panel._doc).val();
					if(url != '' && url != 'http://')
